...
use amiquip::{Exchange, Channel, Publish};
...
pub fn send_stocks(channel: Arc<Mutex<Channel>>, stock_vector: Arc<Mutex<Vec<Stock>>>) {
    let channel = channel.lock().unwrap();
    let exchange = Exchange::direct(&channel);

    let stocks = stock_vector.lock().unwrap();
    
    // Create the formatted stock details
    let stock_details: Vec<String> = stocks.iter()
        .map(|stock| format!(
            "Stock: {}, Quantity: {}, Price: ${:.2}, Buy Rate: ${:.2}, Sell Rate: ${:.2}",
            stock.name, stock.quantity, stock.price, stock.buy_rate, stock.sell_rate
        ))
        .collect();

    // Join the stock details into a single message
    let message = stock_details.join("\n");

    // Send the message
    if let Err(err) = exchange.publish(Publish::new(message.as_bytes(), "stocks")) {
        eprintln!("Failed to send stock data: {}", err);
    } else {
...
pub struct StockTransaction {
    pub name: String,
    pub quantity: u32,
    pub action: String, // "bought" or "sold"
}

#[derive(Debug)]
pub struct StockTracker {
    pub transactions: Vec<StockTransaction>,
    pub stock_stats: HashMap<String, (u32, u32)>, 
    // Stock name -> (bought_quantity, sold_quantity)
}

impl StockTracker {
    // Initialize the StockTracker with a new instance
    pub fn new() -> Self {
        StockTracker {
            transactions: Vec::new(),
            stock_stats: HashMap::new(),
        }
    }

    // Record a stock transaction (buy or sell)
    pub fn record_transaction(&mut self, name: &str, quantity: u32, action: &str) {
        ...
        // Update stock stats (bought and sold quantities)
        let entry = self.stock_stats.entry(name.to_string()).or_insert((0, 0));
        match action {
            "bought" => entry.0 += quantity, // Increment bought quantity
            "sold" => entry.1 += quantity,   // Increment sold quantity
            _ => {}
        }
    }

    // Generate a detailed report of stock transactions
    pub fn generate_report(&self) {
        println!("*** Final Stock Report ***");
        for (name, (bought, sold)) in &self.stock_stats {
            println!("Stock: {}", name);
            println!("  Total Bought: {}", bought);
            println!("  Total Sold: {}", sold);
            println!();
        }
    }
}
